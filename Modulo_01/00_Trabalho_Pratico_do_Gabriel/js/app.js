...
function catalagoDetails(product) {
    return `<section class="product-details"><div class="details-row">
        ...
        <div>Price</div>
        <div class="details-bar">
          <div class="details-bar-bg" style="width= 250">${product.price * 5.5}</div>
        </div>
      </div><div class="details-row">
        <div>Rating</div>
        <div class="details-bar">
    ...
    if (marcasArray.length > 0) {
        for (let index = 0; index < marcasArray.length; index++) {
            filtroMarca.innerHTML =
                filtroMarca.innerHTML + '<option value="' + index + '">' + marcasArray[index] + "</option>";
        }
    }
    ...
    if (tipoArray.length > 0) {
        for (let index = 0; index < tipoArray.length; index++) {
            filtroTipo.innerHTML =
                filtroTipo.innerHTML + '<option value="' + index + '">' + tipoArray[index] + "</option>";
        }
    }
    ...
    if (catalogoArray[0].length > 0) {
        for (produto of catalogoArray[0]) {
            if (catalagoAll === null) {
                ...
                catalagoAll = catalagoAll + catalagoItem(produto);
            }
        }
    }
...
function filtrarClassificacao() {
    ...
    for (marca of marcasFiltro) {
        if (catalagoAll === null) {
            ...
            catalagoAll = catalagoAll + catalagoItem(marca);
        }
    }
...
function filtrarCatalogo() {
    ...
    if (name !== null) {
        catalogoObj = catalogoArray[0].filter((vptMarca) => {
            return vptMarca.name.trim() === name;
        });
    }
    ...
    console.log(catalogoObj.length);
    ...
    // if (classificacao === "Melhor Avaliados") {
    //     catalogoObj.sort((i1, i2) => {
    //         if (i1.id < i2.id) {
    //             return -1;
    //         } else if (i1.id > i2.id) {
    //             return 1;
    ...
    //         }
    //     });
    // } else if (classificacao === "Maiores Preços") {
    //     // catalogoObj = catalogoObj.sort(function (a, b) {
    //     //     return b.price - a.price;
    //     // });
    //     catalogoObj.sort((i1, i2) => {
